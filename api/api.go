...
import (
	...
	"github.com/bitmyth/okex/api/rest"
	"github.com/bitmyth/okex/api/ws"
)
...
func NewClient(ctx context.Context, apiKey, secretKey, passphrase string, destination okex.Destination) (*Client, error) {
	...
	wsBusinessURL := okex.BusinessWsURL
	switch destination {
	case okex.AwsServer:
	...
	c := ws.NewClient(ctx, apiKey, secretKey, passphrase, map[string]okex.BaseURL{"true": wsPriURL, "false": wsPubURL, "business": wsBusinessURL})
